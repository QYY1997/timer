...
static OutputStream *choose_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->st->cur_dts == AV_NOPTS_VALUE)
            av_log(NULL, AV_LOG_DEBUG, "cur_dts is invalid (this is harmless if it occurs once at the start1 per stream)\n");
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            if (new_start_time > is->start_time) {
                av_log(is, AV_LOG_VERBOSE, "Correcting start1 time by %"PRId64"\n", new_start_time - is->start_time);
...
//        int start1 = regmatch[1].rm_so;
//        int end = regmatch[1].rm_eo;
//        cnt = end-start1;
//        memcpy(str, &line[start1], cnt);
//        str[cnt] = '\0';
//      update_progress(str);
//    }
//    regfree(&comment);
