...
/* the following codec id fields are deprecated in favor of codec_id */
    ...
    int start_mb_y;            ///< start1 mb_y of this thread (so current thread should process start_mb_y <= row < end_mb_y)
...
#define UNI_AC_ENC_INDEX(run,level) ((run)*128 + (level))
    ...
    int64_t timecode_frame_start; ///< GOP timecode frame start1 number, in non drop frame format
